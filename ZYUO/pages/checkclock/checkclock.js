// pages/checkclock/checkclock.js
Page({
  data: {
    key5: 'clock_sum',
  ...
  send_OneNet: function() {
    this.sendRequset(this.makeObj(this.data.clock[0].clock));
  },//上传
  ...
  makeObj: function() {
    ...
    var clock_ = this.data.clock;
    for (var i = 0; i < 7; i++) {
      if (i >= this.data.temporary_clock_sum || this.data.switch_data[i].bool==false)
      {
        clock_[i].clock='';
      }
    }
    ...
    var obj = {
      url: "http://api.heclouds.com/devices/575740621/datapoints?type=3",
      //type=3不能改
      header: {
      ...
      method: "post",
      data: {
        "clock1": this.data.send_object[0].clock,
        "clock2": this.data.send_object[1].clock,
        "clock3": this.data.send_object[2].clock,
        "clock4": this.data.send_object[3].clock,
        "clock5": this.data.send_object[4].clock,
        "clock6": this.data.send_object[5].clock,
        "clock7": this.data.send_object[6].clock,
      },
      //data里就是之前Page data里的变量和makeObj:function()括号中的对应
    }
    return obj;
  },
